...
# --- Import and Global Variable for Reset Logic ---
import maguear
import Correo
...
def _reset_session_from_data_runner(reason="Desconocida"):
    global _count_texto_raro_consecutivo
    print(f"\n--- Iniciando reseteo de sesión de magueo desde data_runner por: {reason} ---")

    Correo.send_mail(_last_objeto_seleccionado, f"Reseteo de sesión (desde data_runner): {reason}")

    if hasattr(maguear, 'restart_dofus_and_click_forge'):
        maguear.restart_dofus_and_click_forge()
    else:
        print("Error: La función 'restart_dofus_and_click_forge' no se encontró en el módulo 'maguear'.")
        print("Asegúrate de que 'maguear' esté correctamente importado y contenga esta función.")
        return

    _count_texto_raro_consecutivo = 0
    print("--- Reseteo de sesión de magueo completado desde data_runner ---")
    ...
    if total_celdas_procesadas_actual > 0:
        ...
        if porcentaje_texto_raro >= UMBRAL_PORCENTAJE_TEXTO_RAR_RESET:
            ...
            if _count_texto_raro_consecutivo >= CONTEO_DE_COMPROBACIONES_DE_TEXTO_RARO:
                _reset_session_from_data_runner(reason="Texto OCR ilegible/resolución desalineada (detección basada en porcentaje de celdas 'vacías' o no numéricas)")
                return [], True
        else:
            _count_texto_raro_consecutivo = 0
    else:
        ...
        if _count_texto_raro_consecutivo >= CONTEO_DE_COMPROBACIONES_DE_TEXTO_RARO:
            _reset_session_from_data_runner(reason="No se detectaron celdas para OCR")
            return [], True
